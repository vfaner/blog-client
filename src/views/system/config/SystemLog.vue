<template>
  <el-main>
    <!-- 搜索栏 -->
    <el-form :model="listParam" label-width="80px" :inline="true">
      <el-form-item>
        <el-input placeholder="请输入日志类型" v-model="listParam.type"></el-input>
      </el-form-item>
      <el-form-item>
        <el-button :icon="Search" @click="searchBtn">搜索</el-button>
        <el-button :icon="Close" style="color: #FF7670;" @click="resetBtn">重置</el-button>
      </el-form-item>
    </el-form>
    <!-- 表格 -->
    <el-table :height="tableHeight" :data="logTable.list" border stripe>
      <el-table-column prop="operateType" label="操作类型"></el-table-column>
      <el-table-column prop="operatorName" label="操作人"></el-table-column>
      <el-table-column prop="message" label="日志内容"></el-table-column>
      <el-table-column prop="operateDate" label="操作时间"></el-table-column>
      <el-table-column label="操作" align="center" width="300">
        <template #default="scope">
          <el-button type="danger" size="small" :icon="Delete" @click="deleteBtn(scope.row.id)">删除</el-button>
          <el-button type="primary" size="small" :icon="View" @click="deleteBtn(scope.row.id)">查看详情</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页 -->
    <el-pagination
        @size-change="sizeChange"
        @current-change="currentChange"
        :current-page.sync="listParam.pageNum===0?1:listParam.pageNum"
        :page-sizes="[10, 20, 40, 80, 100]"
        :page-size="listParam.pageSize===0?10:listParam.pageSize"
        layout="total, sizes, prev, pager, next, jumper"
        :total="logTable.total"
    />
  </el-main>
</template>
<script setup lang="ts">
import {ref, nextTick, onMounted} from 'vue';
import {Search, Close, Plus, Delete, View, Setting} from '@element-plus/icons-vue';
import useLogTable from '@/composables/log/useLogTable';
import useLog from "@/composables/log/useLog";

//表格高度
const tableHeight = ref(0);
//表格列表
const {listParam, getLogList, logTable, sizeChange, currentChange, searchBtn, resetBtn} = useLogTable();
//新增，编辑，删除，保存，分配权限
const {deleteBtn} = useLog(getLogList);
onMounted(() => {
  nextTick(() => {
    tableHeight.value = window.innerHeight - 220
  })
})
</script>
<style scoped lang="scss">
</style>
